...
import type { RangeFilter, RangeValue } from "./index";
...
export type RangeSliderAnimation = "opacity" | "height";

export type RangeSliderProps = Omit<SliderProps, "ref"> & {
  value: RangeValue;
  setValue: (value: RangeValue) => void;
  range?: RangeFilter;
  animation?: RangeSliderAnimation;
};
...
export type RangeSliderPipProps = {
  isInRange?: boolean;
  animation?: RangeSliderAnimation;
};

const RangeSliderPip: React.FC<RangeSliderPipProps> = ({
  animation = "height",
  isInRange = true,
}) => {
...
const RangeSlider = React.forwardRef<HTMLDivElement, RangeSliderProps>(
  ({ value, setValue, range, animation, className, ...props }, ref) => {
      ...
      <div className={cn("flex flex-col gap-3", className)}>
        ...
        <div className="flex items-center">
          <Input
            ...
            startContent={<FaRegCalendarAlt className="text-default-400" />}
            type="number"
            value={`${value[0]}`}
